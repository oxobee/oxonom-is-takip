  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-8 gap-3">
      {[
        ...
        { val: rangeTasks.filter((t: Task) => t.data.status !== 'Tamamlandı').length, lbl: 'Beklemedes', icon: <Clock size={15} aria-hidden="true"/>, bg: 'bg-orange-500/10', iconC: 'text-orange-400', sub: `${rangeActiveTasks} activas`, click: 'tasks' },
        { val: overdueCount, lbl: 'Gecikmişs', icon: <AlertTriangle size={15} aria-hidden="true"/>, bg: overdueCount > 0 ? 'bg-red-500/10' : 'bg-[var(--af-bg4)]', iconC: overdueCount > 0 ? 'text-red-400' : 'text-emerald-400', sub: overdueCount > 0 ? 'Requieren atención' : 'Sin vencidas', click: 'tasks', alert: overdueCount > 0 },
