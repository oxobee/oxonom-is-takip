...
export default function DashboardKPIs({
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-8 gap-3">
      {[
        { val: projectsLength, lbl: 'Projeler', icon: <FolderKanban size={15} aria-hidden="true"/>, bg: 'bg-blue-500/10', iconC: 'text-blue-400', sub: `${execProjects} en ejecución`, click: 'projects' },
        { val: rangeTasks.filter((t: Task) => t.data.status !== 'Tamamlandı').length, lbl: 'Bekleyen Görevler', icon: <Clock size={15} aria-hidden="true"/>, bg: 'bg-orange-500/10', iconC: 'text-orange-400', sub: `${rangeActiveTasks} activas`, click: 'tasks' },
        { val: overdueCount, lbl: 'Gecikmişler', icon: <AlertTriangle size={15} aria-hidden="true"/>, bg: overdueCount > 0 ? 'bg-red-500/10' : 'bg-[var(--af-bg4)]', iconC: overdueCount > 0 ? 'text-red-400' : 'text-emerald-400', sub: overdueCount > 0 ? 'Müdahale gerekiyor' : 'Gecikme yok', click: 'tasks', alert: overdueCount > 0 },
        { val: todayMeetingsLength, lbl: 'Bugünkü Toplantılar', icon: <CalendarDays size={15} aria-hidden="true"/>, bg: 'bg-purple-500/10', iconC: 'text-purple-400', sub: weekTasksLength > 0 ? `${weekTasksLength} tareas esta semana` : '', click: 'calendar' },
        { val: openRFIs + pendingSubmittals, lbl: 'RFI & Onaylar', icon: <CircleHelp size={15} aria-hidden="true"/>, bg: 'bg-blue-500/10', iconC: 'text-blue-400', sub: `${openRFIs} RFIs · ${pendingSubmittals} subs`, click: 'rfis', alert: overdueRFIs > 0 },
        { val: openPunchItems, lbl: 'Kusur Listesi', icon: <ListChecks size={15} aria-hidden="true"/>, bg: 'bg-teal-500/10', iconC: 'text-teal-400', sub: `${punchItemsLength} items totales`, click: 'punchList' },
        { val: totalBudget > 0 ? Math.round((totalExpenses / totalBudget) * 100) + '%' : '—', lbl: 'Bütçe', icon: <DollarSign size={15} aria-hidden="true"/>, bg: totalBudget > 0 && totalExpenses > totalBudget * 0.9 ? 'bg-red-500/10' : 'bg-emerald-500/10', iconC: totalBudget > 0 && totalExpenses > totalBudget * 0.9 ? 'text-red-400' : 'text-emerald-400', sub: totalBudget > 0 ? `${fmtCOP(totalExpenses)} / ${fmtCOP(totalBudget)}` : 'Sin presupuesto', click: 'budget', alert: totalBudget > 0 && totalExpenses > totalBudget },
        { val: rangeTotalTime > 0 ? fmtHours(rangeTotalTime) : '—', lbl: 'Çalışma Süresi', icon: <Timer size={15} aria-hidden="true"/>, bg: 'bg-indigo-500/10', iconC: 'text-indigo-400', sub: `${rangeTimeEntriesLength} registros`, click: 'timeTracking' },
      ].map((m, i) => (
