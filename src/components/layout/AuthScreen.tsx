...
function validateLoginForm(forms: Record<string, any>) {
  ...
  if (!email) {
    errors.loginEmail = 'E-posta adresi zorunludur';
  } else if (!EMAIL_REGEX.test(email)) {
    errors.loginEmail = 'Lütfen geçerli bir e-posta adresi girin';
  }

  if (!pass) {
    errors.loginPass = 'Şifre zorunludur';
  } else if (pass.length < 6) {
    errors.loginPass = 'Şifre en az 6 karakter olmalıdır';
  }
...
function validateRegisterForm(forms: Record<string, any>) {
  ...
  if (!name) {
    errors.regName = 'Ad Soyad zorunludur';
  } else if (name.length < 2) {
    errors.regName = 'Ad Soyad en az 2 karakter olmalıdır';
  }

  if (!email) {
    errors.regEmail = 'E-posta adresi zorunludur';
  } else if (!EMAIL_REGEX.test(email)) {
    errors.regEmail = 'Lütfen geçerli bir e-posta adresi girin';
  }

  if (!pass) {
    errors.regPass = 'Şifre zorunludur';
  } else if (pass.length < 6) {
    errors.regPass = 'Şifre en az 6 karakter olmalıdır';
  }
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-[var(--background)] via-[var(--background)] to-[var(--af-bg3)] z-50 flex items-center justify-center p-6">
      ...
      <div className="fixed bottom-2 left-2 z-[60] text-[10px] px-2 py-1 rounded bg-black/60 text-white/70 font-mono">
        Firebase: {fbStatus.status === 'checking' ? '⏳ yükleniyor...' : fbStatus.status === 'ok' ? '✅ Bağlı' : `❌ ${fbStatus.detail}`}
      </div>
      ...
      <div className="relative bg-[var(--card)] border border-[var(--input)] rounded-2xl p-8 w-full max-w-[400px] af-card shadow-2xl">
        <div className="flex items-center gap-3 mb-7">
          ...
          <div className="flex flex-col">
            <span style={{ fontFamily: "'DM Serif Display', serif" }} className="text-2xl leading-none">oxonom</span>
            <span className="text-[11px] font-medium text-[var(--af-accent)] tracking-wide mt-0.5">iş takip platformu</span>
          </div>
        </div>
        ...
        {fbStatus.status === 'error' && (
          <div className="mb-4 p-3 rounded-lg bg-red-500/15 border border-red-500/30 text-sm text-red-400">
            <div className="font-medium mb-1">Firebase bağlantısı kurulamadı</div>
            <div className="text-xs opacity-80">{fbStatus.detail}</div>
            <div className="text-xs opacity-60 mt-1">Detaylar için tarayıcı konsolunu inceleyebilirsiniz.</div>
          </div>
        )}
        ...
        {authLoading && (
          <div className="absolute inset-0 flex items-center justify-center z-10 bg-[var(--background)]/60 backdrop-blur-sm rounded-2xl">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-[11px] text-[var(--muted-foreground)]">Giriş yapılıyor...</span>
            </div>
          </div>
        )}

        {!forms.showRegister ? (<>
          <div className="text-xl font-semibold mb-1">Tekrar Hoş Geldiniz</div>
          <div className="text-sm text-[var(--muted-foreground)] mb-7">Devam etmek için hesabınıza giriş yapın</div>
          <div className="mb-3">
            <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1.5">E-posta adresi</label>
            <input
          ...
          <div className="mb-4">
            <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1.5">Şifre</label>
            <input
          ...
          >{authLoading ? 'Giriş yapılıyor...' : 'Giriş Yap'}</button>
          <div className="flex items-center gap-3 my-4 text-xs text-[var(--af-text3)]"><div className="flex-1 h-px bg-[var(--border)]" />veya<div className="flex-1 h-px bg-[var(--border)]" /></div>
          ...
          <div className="text-center mt-5 text-sm text-[var(--af-text3)]">Hesabınız yok mu? <a className="text-[var(--af-accent)] cursor-pointer hover:underline" onClick={() => setForms(p => ({ ...p, showRegister: true }))}>Kayıt Ol</a></div>
        </>) : (<>
          <div className="text-xl font-semibold mb-1">Yeni Hesap Oluşturun</div>
          <div className="text-sm text-[var(--muted-foreground)] mb-7">oxonom iş takip platformuna katılın</div>
          <div className="mb-3">
            <label className="block text-xs font-medium text-[var(--muted-foreground)] mb-1.5">Ad Soyad</label>
            <input
          ...
          <button
            ...
            disabled={authLoading || fbStatus.status !== 'ok'}
          >{authLoading ? 'Hesap oluşturuluyor...' : 'Hesap Oluştur'}</button>
          <div className="flex items-center gap-3 my-4 text-xs text-[var(--af-text3)]"><div className="flex-1 h-px bg-[var(--border)]" />veya<div className="flex-1 h-px bg-[var(--border)]" /></div>
          <button className="af-btn-secondary w-full flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:cursor-not-allowed" onClick={handleGoogleClick} disabled={authLoading || fbStatus.status !== 'ok'}>
            <svg viewBox="0 0 24 24" className="w-[18px] h-[18px]"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
            Google ile Kayıt Ol
          ...
          <div className="text-center mt-5 text-sm text-[var(--af-text3)]">Zaten hesabınız var mı? <a className="text-[var(--af-accent)] cursor-pointer hover:underline" onClick={() => setForms(p => ({ ...p, showRegister: false }))}>Giriş Yap</a></div>
        </>)}
      </div>
    </div>
  );
