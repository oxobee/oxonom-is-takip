...
export default function InvoicesScreen() {
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      ...
      { lbl: 'Beklemede', val: fmtCOP(totalPending), color: 'text-blue-400' },
      ...
      {invoiceTab === 'list' && (<div>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div className="flex gap-1 bg-[var(--af-bg3)] rounded-lg p-1 overflow-x-auto">
              {[{ k: 'Todas', v: 'all' }, { k: 'Taslak', v: 'Taslak' }, { k: 'Enviadas', v: 'Enviada' }, { k: 'Ödendis', v: 'Ödendi' }, { k: 'Gecikmişs', v: 'Gecikmiş' }].map(tab => (
            ...
            <EmptyState emoji="🧾" title="Sin facturas" description="İlk kaydınızı oluşturun factura para empezar" />
              ...
              <div className="space-y-2">
                {filteredInvoices.map(inv => {
                  const statusColors: Record<string, string> = { Taslak: 'bg-[var(--af-bg4)] text-[var(--muted-foreground)]', Enviada: 'bg-blue-500/10 text-blue-400', Ödendi: 'bg-emerald-500/10 text-emerald-400', Gecikmiş: 'bg-red-500/10 text-red-400', Cancelada: 'bg-red-500/5 text-red-300 line-through' };
                  ...
                  return (<div key={inv.id} className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 flex items-center gap-4 cursor-pointer hover:border-[var(--input)] transition-all">
                    ...
                    <div className="hidden md:flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                      {/* PDF Download */}
                      <button aria-label="PDF İndir" className="px-2 py-1.5 rounded text-xs cursor-pointer bg-[var(--af-accent)]/10 text-[var(--af-accent)] hover:bg-[var(--af-accent)]/20" onClick={() => {
                        try { exportInvoicePDF(inv, proj); showToast('PDF descargado'); } catch (err) { showToast('Error al generar PDF', 'error'); }
                      }} title="PDF İndir">
                        <FileText size={14} aria-hidden="true"/>
                      </button>
                      {inv.data.status === 'Borrador' && <button className="px-2 py-1.5 rounded text-xs cursor-pointer bg-blue-500/10 text-blue-400 hover:bg-blue-500/20" onClick={() => fbActions.updateInvoiceStatus(inv.id, 'Enviada', showToast, activeTenantId)}>Gönder</button>}
                    ...
                    <div className="md:hidden shrink-0" onClick={e => e.stopPropagation()}>
                      <OverflowMenu
                        actions={[
                          { label: 'PDF İndir', icon: <FileText size={14} aria-hidden="true"/>, onClick: () => {
                            try { exportInvoicePDF(inv, proj); showToast('PDF descargado'); } catch (err) { showToast('Error al generar PDF', 'error'); }
                          }},
                          ...(inv.data.status === 'Borrador' ? [{ label: 'Gönder factura', onClick: () => fbActions.updateInvoiceStatus(inv.id, 'Enviada', showToast, activeTenantId) }] : []),
        ...
        {invoiceTab === 'create' && (<div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button className="text-xs text-[var(--muted-foreground)] cursor-pointer hover:underline" onClick={() => setInvoiceTab('list')}>İptal</button>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[var(--af-bg3)] rounded-lg p-3 space-y-2">
              <div className="grid grid-cols-12 gap-2 text-[11px] text-[var(--muted-foreground)] font-medium">
                <div className="col-span-5 sm:col-span-4">Harcama Kalemi</div><div className="col-span-3 sm:col-span-3">Fase</div><div className="col-span-2 sm:col-span-2">Horas</div><div className="col-span-2 sm:col-span-2">Tarifa</div><div className="col-span-1"></div>
              </div>
              {invoiceItems.map((item: any, idx: any) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  <input className="col-span-5 sm:col-span-4 bg-[var(--card)] border border-[var(--border)] rounded px-2 py-1.5 text-xs outline-none" placeholder="Harcama Kalemi" value={item.concept} onChange={e => updateInvoiceItem(idx, 'concept', e.target.value)} />
          ...
          <textarea className="w-full bg-[var(--af-bg3)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--foreground)] outline-none resize-none" rows={2} placeholder="Notlar..." value={forms.invNotes || ''} onChange={e => setForms(p => ({ ...p, invNotes: e.target.value }))} />
